...
把评测口径从贪心换成**训练时真正使用的采样分布**（T=0.8 / top_p=0.95 / rp=1.0），
**同一个合并模型、同一套 vLLM 管线**（2026-09-14 补测）：

| 解码口径 | 基座 | +GRPO 1500 | Δ | McNemar p |
|---|---|---|---|---|
| **贪心** | 966（73.2%）| **1012（76.7%）** | **+3.5** | **0.0009** ✅✅ |
| **采样 T=0.8 / top_p=0.95** | 951（72.1%）| 958（72.6%）| **+0.5** | **0.69** ❌ |

**→ 在训练真正采样的那个分布上，这个增益没能复现出来**（+0.5，p=0.69）。
...
|---|---|---|
| 基座 | −1.1 点 | 0.34（不显著）|
| **+GRPO** | **−4.1 点** | **0.0006**（极显著）|

**RL 模型显著比基座更依赖贪心解码。**

#### ⚠️ 但"增益是贪心专属"这个更强的说法**没有被证明**

按题做配对 bootstrap 检验交互（贪心Δ − 采样Δ）：

```
交互 = +2.96 点    95% bootstrap CI = [+0.00, +5.91]    p ≈ 0.05  ← 刚触到 0
```

**CI 跨 0，交互不显著。** 这正是「显著 vs 不显著，本身未必显著」那个陷阱：
贪心显著 + 采样不显著，**推不出**"效应确实依赖解码"。

所以能说的和不能说的分清楚：

| | 措辞 |
|---|---|
| ✅ 能说 | 贪心下增益显著（+3.5, p=0.0009）；采样口径下**未能复现**（+0.5, p=0.69）；RL 模型自身显著更依赖贪心（−4.1, p=0.0006） |
| ❌ 不能说 | 已**证明**增益只存在于贪心；已证明"分布期望没变" |

> **为什么这个区别重要**：采样比较目前只有 **1 条生成/题**，而效应只有 ~3 点 —— 噪声太大，
> 边缘的交互很可能只是**功效不足**，不是真的没有交互。要判定它必须降方差：
> 用 G=8（同一套脚本已支持）重测采样通过率，方差降约 8 倍。

**机制假设**（有旁证，未当定论）：发现 B 测得 RL 把 4-gram 重复率降了 27%。
贪心最容易卡进重复循环，而 top_p=0.95 采样本来就能自己跳出来 ——
**如果**交互成立，机制就是：RL 学到的是「在贪心模式下不卡住」，而不是「分布整体更对」。
换句话说：**RLVR 锐化了策略的众数（argmax 变好），而分布的期望准确率几乎没动。**
