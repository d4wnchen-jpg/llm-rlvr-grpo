...
| vLLM reports `FileNotFoundError: 'ninja'` | The `ninja` package was installed, but the venv's `bin` directory is not on `PATH` when calling the interpreter by absolute path without activating. Fix: `ln -sf /root/venv-vllm/bin/ninja /usr/local/bin/ninja` |
